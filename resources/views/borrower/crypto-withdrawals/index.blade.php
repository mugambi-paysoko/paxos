...
@section('page_actions')
    <a href="{{ route('borrower.profiles.index') }}" class="btn btn-outline-primary rounded-3">
...
@section('content')
                ...
                <tbody>
                    @forelse ($cryptoWithdrawals as $cryptoWithdrawal)
                        <tr>
                            ...
                            <td class="text-end">
                                <a href="{{ route('borrower.crypto-withdrawals.show', $cryptoWithdrawal) }}" class="btn btn-sm btn-outline-primary rounded-3">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    @if ($__hasActiveCryptoWithdrawals)
        <x-status-poll
            mode="digest"
            :url="route('borrower.status.crypto-withdrawals.digest')"
