...
@section('page_actions')
    ...
    @endif
    <a href="{{ route('lender.fiat-deposit-instructions.index') }}" class="btn btn-outline-secondary rounded-3">
        <i class="ti ti-arrow-left me-1"></i> All instructions
    </a>
@endsection
        ...
        <div class="col-md-4">
            <div class="card shadow-sm border-0 rounded-4 h-100">
                <div class="card-body p-4">
                    ...
                    <span class="badge rounded-pill {{ $fiatDepositInstruction->status === 'VALID' ? 'text-bg-success' : 'text-bg-warning' }}">{{ $fiatDepositInstruction->status ?? 'N/A' }}</span>
        ...
        <div class="card shadow-sm border-0 rounded-4">
            <div class="card-body p-4 p-md-5">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="fw-semibold mb-0">Related deposits</h5>
                    <a href="{{ route('lender.fiat-deposits.index') }}" class="btn btn-sm btn-outline-primary rounded-3">View all</a>
                </div>
                <div class="table-responsive">
                    <table class="table align-middle mb-0">
                        <thead class="table-light">
                        ...
                        <tbody>
                            @foreach ($fiatDepositInstruction->fiatDeposits as $deposit)
                                <tr>
                                    <td class="fw-semibold text-primary">{{ $deposit->asset }} {{ number_format($deposit->amount, 2) }}</td>
                                    <td>
                                        <span class="badge rounded-pill {{ $deposit->status === 'COMPLETED' ? 'text-bg-success' : ($deposit->status === 'PENDING' ? 'text-bg-warning' : 'text-bg-secondary') }}">{{ $deposit->status ?? 'PENDING' }}</span>
                                    </td>
                                    <td class="text-muted">{{ $deposit->created_at->diffForHumans() }}</td>
                                    <td class="text-end">
                                        <a href="{{ route('lender.fiat-deposits.show', $deposit) }}" class="btn btn-sm btn-outline-primary rounded-3">View</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
