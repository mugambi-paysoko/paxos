...
@section('page_actions')
    <a href="{{ route('lender.fiat-withdrawals.index') }}" class="btn btn-outline-secondary rounded-3">
...
@php
    $__lfw = strtoupper((string) ($fiatWithdrawal->status ?? 'PENDING'));
@endphp
@if (! in_array($__lfw, ['COMPLETED', 'FAILED', 'CANCELLED', 'REJECTED', 'SETTLED', 'RETURNED'], true))
    <x-status-poll
        mode="transfer"
        :url="route('lender.status.fiat-withdrawal', $fiatWithdrawal)"
